...
class DarkCloudCoverDetector(DoubleCandlePatternDetector):
    ...
    def __init__(
        ...
        *,
        ...
        require_gap_up_into_c2: bool = True,         # classic requires a gap up into candle 2
        gap_tolerance: float = 1e-9,                 # lenient band for gap check (absolute tolerance)
        require_close_below_midpoint1: bool = True,  # c2 <= midpoint(o1, c1)
        require_close_above_o1: bool = True,         # c2 >= o1 (classic definition)
        midpoint_margin_ratio: float = 0.0,          # extra margin below midpoint: margin = body1 * ratio

        # Strength constraint (relative)
        min_body2_ratio_vs_body1: float = 0.80,      # body2 >= 0.8 * body1
    ...
    def detect(
        ...
        **overrides
        ...
        # (1) Gap up into candle 2 (o2 > c1), with optional tolerance
        gap_up_ok = True
        if p["require_gap_up_into_c2"]:
            gap_up_ok = o2 >= (c1 * (1 + p["gap_tolerance"]))

        # (2) Close below midpoint of candle 1 (optionally require margin deeper below)
        midpoint_ok = True
        ...
        # (3) Close above candle 1 open (classic DCC: not a full engulfing)
        close_above_o1_ok = True
        if p["require_close_above_o1"]:
            close_above_o1_ok = c2 >= (o1 * (1 - p["float_tolerance"]))

        # (4) Strength of body2 relative to body1 (optional)
        strength_ok = True
