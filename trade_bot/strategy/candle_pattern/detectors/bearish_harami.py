...
class BearishHaramiDetector(DoubleCandlePatternDetector):
    ...
    Bearish Harami (2-candle):
      ...
      - Candle 2 body lies fully inside Candle 1 body (for bullish Candle 1: o2 <= c1 and c2 >= o1)
      - Typically appears after an uptrend (trend/location/volume confirmation recommended externally)
    """
    def __init__(
        self,
        *,
        ...
        inside_tolerance: float = 1e-9,       # Lenient absolute tolerance when strict_inside=False
    ...
    def detect(
        ...
        **overrides
        ...
        if p["strict_inside"]:
            # For bullish candle1, the second body must be fully inside the first body
            ...
            inside_ok = (o2 <= (c1 + abs(c1) * p["inside_tolerance"])) and \
                        (c2 >= (o1 - abs(o1) * p["inside_tolerance"]))
