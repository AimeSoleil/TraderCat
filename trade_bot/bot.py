...
from trade_bot.strategy.bbands_breakout_strategy import (
    BollingerBreakoutStrategy,
    make_bbands_breakout_presets,
)
from trade_bot.strategy.bbands_reversal_strategy import (
    BBandsReversalStrategy,
    make_bbands_reversal_presets,
)
from trade_bot.strategy.candlestick_reversal_strategy import (
...
from trade_bot.strategy.fibonacci_retracement_strategy import (
    FibonacciRetracementStrategy,
    make_fibonacci_presets,
)
from trade_bot.strategy.momentum_strategy import (
...
class TradeBot:
    """Trade bot for a specific symbol."""
    ...
    async def run(self):
        ...
        strategies_per_symbol = [
            BollingerBreakoutStrategy(
                data_provider=data_provider, **make_bbands_breakout_presets()["swing"]
            ),
            BBandsReversalStrategy(
                data_provider=data_provider, **make_bbands_reversal_presets()["swing"]
            ),
            DivergenceStrategy(
                data_provider=data_provider, **make_divergence_presets()["swing"]
            ),
            CandlestickReversalStrategy(
                data_provider=data_provider,
                **make_candlestick_reversal_presets()["swing"],
            ),
            FibonacciRetracementStrategy(
                data_provider=data_provider, **make_fibonacci_presets()["swing"]
            ),
            MomentumTrendStrategy(
                data_provider=data_provider, **make_momentum_presets()["swing"]
            ),
        ]
        ...
        max_lookback = max(
            strategy.get_lookback_window() for strategy in strategies_per_symbol
        )
        candles = data_provider.get_price_data(
            self.symbol, interval="1d", lookback=max_lookback
        )
        logger.info(
            f"Fetched {len(candles)} candles for {self.symbol}: {candles[-1] if candles else 'No candles'}"
        )
...
class GlobalTradeBot:
    """Trade bot for global strategies that do not depend on a specific symbol."""

    def __init__(self, executor):
        self.executor = executor

    async def run(self):
        logger.info(f"Running global bot...")
        data_provider = OpenBBProvider()
