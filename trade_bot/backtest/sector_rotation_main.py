...
def run_sector_rotation_backtest(
    ...
    if config['universe'] == 'sub_sector':
        from trade_bot.strategy.sector_rotation_strategy import SUB_SECTOR_LIST
        universe_map = SUB_SECTOR_LIST
    else:
        from trade_bot.strategy.sector_rotation_strategy import GICS_SECTOR_LIST
    ...
    for current_date in dates:
        ...
        if signal.signal == "rebalance":
            ...
            logger.info(f"[{current_date.date()}] Val: ${portfolio_value:.0f} | {signal.details.get('regime')} | {selected_symbols}")
