...
class ThreeBlackCrowsDetector(TripleCandlePatternDetector):
    """
    Three Black Crows (Bearish Reversal) - US Stock Optimized:
        - Pattern: Three consecutive long bearish candles.
        - Structure: Staircase down (Lower Highs, Lower Lows, Lower Closes).
        - Psychology: Panic selling or liquidation. Bulls have completely given up.
        - Optimization: Enforces strong bodies and weak lower wicks to filter out "slow drifts" or "dip buying".
    """
    ...
    def __init__(
        ...
        *,
        ...
        # [Optimization] True. 
        # In a strong crash, lows should also be moving down (Staircase pattern).
        require_lower_lows: bool = True,

        # --- Open/Gap Logic ---
        # [Optimization] False. 
        # Textbook says "Open within previous body", but in US Stocks, 
        # a Gap Down (Open < Prev Close) indicates even stronger panic. 
        # We disable this to capture high-momentum crashes.
        ...
        # [Optimization] True. 
        # The first crow initiates the reversal; it must be significant.
        require_big_first_candle: bool = True,  

        # --- Range-based Body Constraints ---
        # [Optimization] 0.40 (40%). 
        # This is the most important filter. 
        # It ensures the candles are "Long Black Candles" (Body is >40% of total range),
        # filtering out small dojis or spinning tops drifting lower.
        min_body_ratio_vs_range: Optional[float] = 0.40,  

        # --- Shadow Constraints (Rejection) ---
        # [Optimization] None. 
        # Upper wicks matter less in a panic sell-off (bulls tried but failed).
        max_upper_shadow_to_body: Optional[float] = None,
        
        # [Optimization] 0.20 (20%). 
        # Tightened from 0.5. Crows should close near their lows. 
        # Long lower wicks indicate buying support ("Buying the dip"), which invalidates the pattern.
        max_lower_shadow_to_body: Optional[float] = 0.20,  

        # --- Volume Logic ---
        # [Optimization] False (Default), but True recommended in config.
        # Ideally, Volume 1 < Volume 2 < Volume 3 (Panic growing).
    ...
    def detect(
        ...
        **overrides
        ...
        # 1) Direction (Bearish)
        if p["require_consecutive_bearish"]:
            if self.is_bullish(o1, c1) or self.is_bullish(o2, c2) or self.is_bullish(o3, c3):
                return PatternResult(is_pattern=False)

        # 2) Bodies Calculations
        ...
        # 4) Open within previous body Logic
        ...
        if p["require_big_first_candle"]:
            if body1 < avg_body: # Simple comparison
                return PatternResult(is_pattern=False)
        ...
        for (o, c, h, l, b, r) in candle_data:
            ...
            # Body vs Range (using adaptive ratio)
        ...
        return PatternResult(
            is_pattern=True, name="Three Black Crows", bias="short", metrics=metrics
        )